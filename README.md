...
**Status:** v0.4.5 released. The current release includes audit labels,
...
fuzz targets. v0.4.5 keeps WAF behavior compatible while bounding repeated
signature findings, accelerating large reputation deny lists, supervising the
relay task graph, accepting documented deny-list address forms, consolidating
the relay's random-number dependency, and strengthening benchmark isolation.
...
```bash
gh release download v0.4.5 --repo guaracloud/purple-wolf --dir purple-wolf-release
```
