    ...
    STATE.with(|s| {
        let (cfg, engine_) = s.get_or_init(|| {
            let cfg = adapter::parse(&host::config()).unwrap_or_else(|e| {
                ...
                // doesn't bring down every route on a bad config. We
                // construct this directly rather than deserializing because
                // `Config` has required fields with no serde defaults.
                Config {
                    mode: Mode::Monitor,
